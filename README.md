![header](https://capsule-render.vercel.app/api?type=speech&reversal=true&color=auto&height=300&section=header&text=Welcome!&fontSize=90)
---

<img src="./images/profile.jpg" width="200">

# 👩‍💻 김영주 (Youngju Kim)  
**단국대학교 정보통계학과 32230894 | Data Analysis & Machine Learning**
  * 📧 E-mail: yoyoung5611@dankook.ac.kr
  * 📱 Phone: 010-3315-6259
  * 🌐 Github: https://github.com/0zooooo


---


## 🌟 자기소개

💓 mbti : INFP

✏️ 정보통계학과에서 3학년 재학중인 데이터 분석가 & 사이언스를 목표로 공부 중인 김영주입니다.

🌱 Python과 R을 활용해 다양한 데이터 분석·시각화·머신러닝 프로젝트를 진행하며 여러 모델들을 학습하고, 데이터를 분석하는 역량을 키우고 있습니다.

🖥️ 웹프로그래밍, 🎮 게임프로그래밍 등에도 관심을 갖고있어 html+css+js를 간단한 프로젝트를 통해 공부하고있고 이후 C++, Unity 등에 대해서 독학하며 간단한 인디게임 프로젝트를 진행해 볼 예정입니다.  

---

## 💻 Tech Stack
### 🔹 Languages  
- ![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-005C84)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)


### 🔹 Tools  
- ![RStudio](https://img.shields.io/badge/RStudio-75AADB)
![VSCode](https://img.shields.io/badge/VSCode-007ACC?logo=visual-studio-code&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?logo=git&logoColor=white)
![PyCharm](https://img.shields.io/badge/PyCharm-000000?logo=PyCharm&logoColor=white)


---

## 📌 참여 Projects

### 🔹 데이터분석 / 모델링 Projects

- **갑상선 암 예측 분류**  
  ✅ 환자 데이터셋 기반 이진 분류 모델 구축 & 예측 성능 최적화   
  *#KNN #Naive Bayes #Decision Tree #데이터마이닝 #이진분류 #ML*

  - ![R](https://img.shields.io/badge/R-276DC3?logo=r&logoColor=white) ![RStudio](https://img.shields.io/badge/RStudio-75AADB)

- **운동 여부가 심혈관에 미치는 영향 분석 & 모델링**  
  ✅ 운동 여부와 심혈관 질환 발생 위험 간의 상관성 분석 & 모델링  
  *#EDA #로지스틱회귀 #헬스통계 #통계분석*  

  - ![R](https://img.shields.io/badge/R-276DC3?logo=r&logoColor=white) ![RStudio](https://img.shields.io/badge/RStudio-75AADB)

---


### 🔹 기타 Projects

<p align="left">
  <img src="./images/web1.png" width="200">
  <img src="./images/web2.png" width="200">
</p>

- **City Tour 웹페이지 제작**
  
  ✅ HTML + CSS + JS 로 구현한 반응형 도시 소개 웹페이지  
  🔗 Repo: https://github.com/0zooooo/T0_Projects

  *#HTML #CSS #JavaScript #WebDesign #Frontend*

  - ![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black) ![VSCode](https://img.shields.io/badge/VSCode-007ACC?logo=visual-studio-code&logoColor=white)

<img src="./images/game.png" width="200">

- **간단 눈·코·입 인물 맞추기 게임 제작**
  
  ✅ 유명 인물의 눈·코·입만 보고 인물 맞추는 tkinter 미니게임  
  *#Python #PyCharm #tkinter*

  - ![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white) ![PyCharm](https://img.shields.io/badge/PyCharm-000000?logo=PyCharm&logoColor=white)

---

## 🏅 자격증
| 취득일 | 활동 | 내용 |
|------|------|------|
| 2025-03-01 | ADSP | 데이터 분석, 통계기초, R기본
| 2025-04-04 | SQLD | SQL 기본
| 2025-12-24| 사회조사분석사 2급 |여론조사, SPSS 통계 처리

---

## 🎯 학습 목표
- [ ] R & python 역량 강화
- [ ] 수리통계학 심화 공부
- [ ] 머신러닝 & 통계모델링 이론 습득 및 실습
- [ ] Spark, Airflow, Tableau 등 여러 데이터 실무 관련 오픈소스 공부
- [ ] 어학 공부 (TOFLE, OPIC)
- [ ] 공모전 & 대외활동 & 자격증(정처기, 빅분기) & 인턴쉽
- [ ] 기타 프로그래밍밍 (웹, 게임 관련) 공부
